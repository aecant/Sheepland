...
public class TesseraView extends BackgroundPanel {
	...
	private static final String PNG_SOLDI = "soldi.png";
	...
	private void disegnaCosto() {
		for (int i = 0; i < costo; i++) {
			panelCosto.add((new JLabel(new ImageIcon(Toolkit.getDefaultToolkit().getImage(CostantiGui.PERCORSO_IMMAGINI + PNG_SOLDI)
	...
	private void aggiornaCosto() {
		...
		if (!tess.equals(Mazzo.TESSERA_FINITA)) {
			...
			panelCosto.removeAll();
			disegnaCosto();
			setBorder(BorderFactory.createMatteBorder(5, 5, 5, 5, CostantiGui.COLORE_TESSERA_FINITA));
		}
	}
